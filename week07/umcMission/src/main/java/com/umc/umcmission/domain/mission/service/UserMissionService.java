...
public class UserMissionService {
  ...
  @Transactional(readOnly = true)
  public PageResDTO.PageRes<MissionResDTO.MissionPreview> getMyInProgressMissions(Long userId, MissionReqDTO.GetMyMissionsReq request) {
    if (!userRepository.existsById(userId)) {
      throw new ProjectException(UserErrorCode.USER_NOT_FOUND);
    }
    ...
    return PageResDTO.PageRes.of(
        userMissionRepository.findByUserIdAndStatus(userId, MissionStatus.IN_PROGRESS, pageable)
            .map(this::toMissionPreview)
    );
  }
